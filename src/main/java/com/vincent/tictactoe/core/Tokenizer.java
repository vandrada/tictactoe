 ...
 * tokens are required to be submitted along with a move in tic-tac-toe. The
 * seed for the tokenizer is constructed from the Player's name and the
 * current time.
 */
public class Tokenizer {
    ...
    public Tokenizer(Player player) {
        ...
        Date date = new Date();
        seed.append(player.getName());
        seed.append(formatter.format(date));
    ...
    /**
     * Returns a token created by the Player's hash code
     * @return a random String to be used as their token
     */
